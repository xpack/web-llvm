...
Distributing mass in a loop is a reverse-post-order traversal through the loop. Start by assigning full mass to the <a href="/docs/api/classes/llvm/loop">Loop</a> header. For each node in the loop: <CodeBlock>- Fetch and categorize the weight distribution for its successors.
